...
while True:
    ...
    # Upload to Internet Archive (must be logged in)
      ...
      for i in range(output_parts):
        outpath_ps1 = f'{os.path.splitext(output_csv)[0]}{i+1}.ps1' if output_parts > 1 else f'{os.path.splitext(output_csv)[0]}.ps1'       
        outpath_csv = f'{os.path.splitext(output_csv)[0]}{i+1}.csv' if output_parts > 1 else output_csv
        with open(outpath_ps1, "w", encoding='UTF-8') as f:
